...
class Batch_TrainData_Generator(object):
    def __init__(self,train_ill,ent_ids1,ent_ids2,batch_size,neg_num):
    ...
    def train_index_gene(self):
        ...
        for pe1,pe2 in self.ent_ill:
            for _ in range(self.neg_num):
                if np.random.rand() <= 0.5:
                    #e1
                    ne1 = self.ent_ids1[np.random.randint(21856)]
                    ne2 = pe2
                else:
                    ne1 = pe1
                    ne2 = self.ent_ids2[np.random.randint(1023418)]
                #same check
                if pe1!=ne1 or pe2!=ne2:
                    train_index.append([pe1,pe2,ne1,ne2])
        #np.random.shuffle(train_index)
    ...
    def next(self):
        if self.iter_count < self.batch_num:
            ...
            positive_sample=[(pe1,pe2) for pe1,pe2,ne1,ne2 in batch_data]
            negative_sample=[(ne1,ne2) for pe1,pe2,ne1,ne2 in batch_data]
