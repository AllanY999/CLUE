...
def cos_sim_mat_generate(emb1,emb2,bs = 128,cuda_num = 1):
    """
    ...
    res_mat = torch.matmul(array_emb1,array_emb2.t())
    return res_mat
...
def batch_mat_mm(mat1,mat2,cuda_num,bs=128):
    ...
    for i in range(0,axis_0,bs):
        temp_div_mat_1 = mat1[i:min(i+bs,axis_0)].cuda()
        res = temp_div_mat_1.mm(mat2.cuda())
        res_mat.append(res)
...
def batch_topk(mat,bs=128,topn = 50,largest = False,cuda_num = 0):
    ...
    for i in range(0,axis_0,bs):
        ...
        res_score.append(score_mat)
        res_index.append(index_mat)
